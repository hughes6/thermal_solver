...
if [[ "$mode" == "--multirate" ]]; then
    ...
    adaptive_airflow_refresh()
    {
        ...
        previous_flows=()
        while true; do
            refresh_target=$(awk -v a="$current" -v d="1" -v start="$refresh_start" -v maximum="5" -v end="$requested_end" 'BEGIN { x=a+d; limit=start+maximum; if(x>limit)x=limit; if(x>end)x=end; print x }')
            ...
            if awk -v a="$refresh_elapsed" -v b="5" 'BEGIN { exit !(a>=b) }'; then
                echo "Airflow refresh failed to converge within 5 s." >&2
                return 3
            fi
        done
    }
    ...
    adaptive_initial_airflow()
    {
        ...
        initial_limit=$(awk -v maximum="2" -v end="$requested_end" 'BEGIN { print (maximum<end?maximum:end) }')
        previous_flows=()
        ...
        if ! awk -v a="$current" -v b="$requested_end" 'BEGIN { exit !(a>=b) }'; then
            echo "Initial airflow failed to converge before the airflow_warmup_time safety limit of 2 s." >&2
            return 3
        fi
    }
    ...
    while awk -v a="$current" -v b="$requested_end" 'BEGIN { exit !(a<b) }'; do
        frozen_target=$(awk -v a="$current" -v d="5" -v b="$requested_end" 'BEGIN { x=a+d; print (x<b ? x : b) }')
...
if [[ "$mode" == "--warm-start" ]]; then
    ...
    "$foam_launcher" foamDictionary "$case_dir/system/controlDict" -entry writeInterval -set 5
