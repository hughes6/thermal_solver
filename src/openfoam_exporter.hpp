...
struct OpenFoamExportOptions {
    ...
    // Applied to bulk-fluid average and per-component maximum drift.
...
private:
    ...
    static void write_semi_frozen_solver_build_assets(
        ...
        const std::string launcher=
            "#!/usr/bin/env bash\n"
            "# Build only when no compatible case-bound solver is installed.\n"
            "set -euo pipefail\n"
            "case_dir=\"$(cd \"$(dirname \"$(readlink -f \"$0\")\")\" && pwd)\"\n"
            "bundle=\"$case_dir/solver_build_bundle\"\n"
            "attester=\"$bundle/tools/openfoam_semifrozen_attestation.py\"\n"
            "if (( $# == 0 )) && [[ -f \"$attester\" ]] && "
                "[[ -n \"${FOAM_API:-}\" && -n \"${WM_PROJECT_VERSION:-}\" && "
                "-n \"${WM_OPTIONS:-}\" ]]; then\n"
            "    expected_sha=$(python3 \"$attester\" --repo-root \"$bundle\" "
                "--print-source-sha 2>/dev/null || true)\n"
            "    solver=$(command -v semiFrozenChtMultiRegionFoam || true)\n"
            "    if [[ \"$expected_sha\" =~ ^[0-9a-f]{64}$ && -n \"$solver\" && "
                "-f \"$solver\" && -x \"$solver\" && ! -L \"$solver\" ]]; then\n"
            "        observed=$(\"$solver\" --thermal-sim-attest 2>/dev/null || true)\n"
            "        expected=\"THERMAL_SIM_SOLVER_ATTESTATION_V1 "
                "solver=semiFrozenChtMultiRegionFoam project_source_sha256=$expected_sha "
                "policy=THERMAL_SIM_SEMIFROZEN_MODE_POLICY_V1 foam_api=$FOAM_API "
                "wm_project_version=$WM_PROJECT_VERSION wm_options=$WM_OPTIONS\"\n"
            "        if [[ \"$observed\" == \"$expected\" ]]; then\n"
            "            echo \"Compatible semi-frozen solver already installed; build skipped.\"\n"
            "            exit 0\n"
            "        fi\n"
            "    fi\n"
            "fi\n"
    ...
    static void write_control_dict(
        ...
        if(options.use_k_omega_sst)
            output <<
                ...
                "        libs (fieldFunctionObjects);\n"
                "        region fluid;\n"
                "        // Write yPlus only with complete solver field output.\n"
                "        // Independent diagnostic times are not restartable.\n"
...
functions
{
    ...
    static void write_region_preparation_script(
        ...
        output <<
            ...
            "failed_checks=$(awk '/^Failed [1-9][0-9]* mesh checks/ "
                "{ total += $2 } END { print total+0 }' \"$check_mesh_log\")\n"
            "determinant_failures=$(grep -Ec "
                "'^[[:space:]]*\\*\\*\\*Cells with small determinant' "
                "\"$check_mesh_log\" || true)\n"
    ...
    static void write_parallel_run_script(
        ...
        output <<
            ...
            "        status=$?\n"
            "        set -e\n"
            "        # A first Ctrl+C is handled by changing stopAt to writeNow.\n"
            "        # wait may be interrupted before the solver has observed that\n"
            "        # control change, so keep waiting for its clean checkpoint exit.\n"
            "        if [[ \"$graceful_stop_requested\" == true ]] && kill -0 \"$active_child_pid\" 2>/dev/null; then\n"
            "            sleep 0.1\n"
            "            continue\n"
            "        fi\n"
            "        break\n"
            ...
            "    active_child_pid=\"\"\n"
            "    active_child_uses_group=false\n"
            "    if [[ \"$graceful_stop_requested\" == true ]]; then\n"
            "        echo \"Checkpoint stop completed. Resume from latestTime by rerunning this command.\"\n"
            "        return 125\n"
            "    fi\n"
            "    return \"$status\"\n"
            "}\n"
            ...
            "{\n"
            "    local signal=\"$1\" control_dict=\"$case_dir/system/controlDict\"\n"
            "    [[ \"$graceful_stop_requested\" == true ]] && return 1\n"
            "    [[ -n \"$active_child_pid\" && -f \"$control_dict\" ]] || return 1\n"
            "    graceful_stop_requested=true\n"
            "    echo \"Stop requested ($signal): writing a restartable checkpoint at the next solver timestep. Press Ctrl+C again only to force termination.\"\n"
            "    if declare -F summary >/dev/null 2>&1; then\n"
            "        summary \"checkpoint_stop_requested signal=$signal\"\n"
            "    fi\n"
            "    # A writeNow event obeys purgeWrite. Disable that purge for this\n"
            "    # one controlled stop so taking a checkpoint cannot silently\n"
            "    # discard the prior restart history. restore_run_state restores\n"
            "    # the configured retention for the next launch.\n"
            "    if ! \"$foam_launcher\" foamDictionary -precision 17 \"$control_dict\" -entry purgeWrite -set 0; then\n"
            "        echo \"Unable to disable checkpoint pruning for the requested stop; forcing termination instead.\" >&2\n"
            "        graceful_stop_requested=false\n"
            "        return 1\n"
            "    fi\n"
            "    if ! \"$foam_launcher\" foamDictionary -precision 17 \"$control_dict\" -entry stopAt -set writeNow; then\n"
            "        echo \"Unable to request a writeNow checkpoint; forcing termination instead.\" >&2\n"
            "        graceful_stop_requested=false\n"
            "        return 1\n"
            "    fi\n"
            "    return 0\n"
            ...
            "    # First Ctrl+C/SIGTERM is a controlled checkpoint request. A\n"
            "    # second signal follows the force-stop path below.\n"
            "    if request_checkpoint_stop \"$signal\"; then\n"
            "        return 0\n"
            "    fi\n"
            "    trap - EXIT\n"
            "    trap '' INT TERM\n"
            ...
            << "}\"\n"
            "checkpoint_retention=\"${THERMAL_SAVED_TIME_DIRECTORIES:-"
            ...
            "    exit 2\n"
            "fi\n"
            "unqualified_thermal=false\n"
            "if [[ \"$mode\" == --unqualified-thermal ]]; then\n"
            "    [[ -f \"$case_dir/.unqualified_airflow_imported\" && -f \"$case_dir/.cold_flow_seed_import_manifest\" ]] || { echo 'Import an unqualified test branch first.' >&2; exit 2; }\n"
            "    unqualified_thermal=true\n"
            "    mode=--multirate\n"
            "    echo 'UNQUALIFIED THERMAL TEST: airflow held; acceptance and airflow refresh disabled.'\n"
            "elif [[ -f \"$case_dir/.unqualified_airflow_imported\" ]]; then\n"
            "    echo 'This is an unqualified test branch. Use --unqualified-thermal explicitly.' >&2; exit 2\n"
            "fi\n"
            "if [[ \"$mode\" != \"run\" && \"$mode\" != \"--warm-start\" "
                "&& \"$mode\" != \"--multirate\" "
                "&& \"$mode\" != \"--cold-flow-seed\" ]]; then\n"
            "    echo \"Usage: $0 [processes] "
                "[--warm-start|--multirate|--cold-flow-seed|--unqualified-thermal [end-time] "
            ...
            << (options.use_multirate_thermal
                ...
                : "if [[ \"$mode\" != \"run\" ]]; then\n"
                  ...
                  "    exit 2\n"
                  "fi\n") <<
            "if { [[ \"$mode\" == \"--cold-flow-seed\" ]] || [[ \"$unqualified_thermal\" == true ]]; } && [[ "
            << (options.use_adaptive_airflow_refresh ? "true" : "false")
            << " != true ]]; then\n"
            "    echo \"Cold-flow seed and unqualified import modes require an adaptive-airflow-capable export.\" >&2\n"
            "    exit 2\n"
            "fi\n"
            "if [[ \"$mode\" != \"run\" ]] && { "
            ...
            "    exit 2\n"
            "fi\n\n"
            "if ! [[ \"$checkpoint_retention\" =~ ^([2-9]|[1-9][0-9]+)$ ]]; then\n"
            "    echo \"THERMAL_SAVED_TIME_DIRECTORIES must be an integer of at least 2.\" >&2\n"
            "    exit 2\n"
            "fi\n\n"
        ...
        output <<
            ...
            << " >/dev/null 2>&1 || true\n"
            "        \"$foam_launcher\" foamDictionary -precision 17 "
                "\"$case_dir/system/controlDict\" -entry purgeWrite -set "
                "\"$checkpoint_retention\" >/dev/null 2>&1 || true\n"
            "    fi\n"
            "}\n"
            "apply_checkpoint_retention()\n"
            "{\n"
            "    if [[ -f \"$case_dir/system/controlDict\" ]]; then\n"
            "        \"$foam_launcher\" foamDictionary -precision 17 \"$case_dir/system/controlDict\" -entry purgeWrite -set \"$checkpoint_retention\"\n"
            "        echo \"Restart checkpoint retention: 0 plus the latest $checkpoint_retention completed time directories.\"\n"
            "    fi\n"
            "}\n"
            "run_lock=\"$case_dir/.thermal_solver_run.lock\"\n"
            "if ! command -v flock >/dev/null 2>&1; then\n"
        ...
        output <<
            ...
            "root_time_complete()\n"
            "{\n"
            "    local candidate=\"$1\" field time_dir region_dir region\n"
            "    time_dir=\"$case_dir/$candidate\"\n"
            "    [[ -d \"$time_dir/fluid\" ]] || return 1\n"
            "    for field in T U p p_rgh phi rho k omega nut alphat; do\n"
            "        [[ -f \"$time_dir/fluid/$field\" ]] || return 1\n"
            "    done\n"
            "    for region_dir in \"$case_dir/constant\"/*; do\n"
            "        [[ -d \"$region_dir/polyMesh\" ]] || continue\n"
            "        region=\"${region_dir##*/}\"\n"
            "        [[ \"$region\" == fluid ]] && continue\n"
            "        [[ -f \"$time_dir/$region/T\" ]] || return 1\n"
            ...
            "    return 0\n"
            "}\n\n"
            "same_checkpoint_time()\n"
            "{\n"
            "    local first=\"$1\" second=\"$2\"\n"
            "    awk -v a=\"$first\" -v b=\"$second\" 'BEGIN { \n"
                "d=a-b; if(d<0)d=-d; scale=(a<0?-a:a); other=(b<0?-b:b); \n"
                "if(other>scale)scale=other; if(scale<1)scale=1; \n"
                "exit !(d<=1e-9*scale) }'\n"
            "}\n\n"
            "ensure_processor_time_metadata()\n"
            "{\n"
            "    local candidate=\"$1\" rank root_metadata target_metadata\n"
            "    if ! awk -v t=\"$candidate\" 'BEGIN { exit !(t>1e-12) }'; then\n"
            "        return 0\n"
            "    fi\n"
            "    root_metadata=\"$case_dir/$candidate/uniform/time\"\n"
            "    if [[ ! -f \"$root_metadata\" ]]; then\n"
            "        echo \"Verified reconstructed checkpoint $candidate has no "
                "root uniform/time metadata. Preserving the new partitions and "
                "refusing to start a solver stage.\" >&2\n"
            "        return 1\n"
            "    fi\n"
            "    for ((rank=0; rank<processes; ++rank)); do\n"
            "        target_metadata=\"$case_dir/processor${rank}/${candidate}/uniform/time\"\n"
            "        [[ -f \"$target_metadata\" ]] && continue\n"
            "        mkdir -p \"${target_metadata%/*}\" || return 1\n"
            "        cp -p \"$root_metadata\" \"$target_metadata\" || return 1\n"
            "    done\n"
            "}\n\n"
            ...
            "        if ! processor_time_complete \"$processor_latest\" "
                "\"$existing_processes\"; then\n"
            "            echo \"Cannot change process count: no complete "
                "checkpoint exists across all $existing_processes existing "
                "processor partitions. Old partitions were preserved.\" >&2\n"
            "            exit 10\n"
            "        fi\n"
            "        if ! root_time_complete \"$root_latest\" || "
                "! same_checkpoint_time \"$root_latest\" \"$processor_latest\"; then\n"
            "        echo \"Reconstructing complete parallel checkpoint "
                "t=$processor_latest from $existing_processes to requested "
            ...
            "        if ! root_time_complete \"$root_latest\" || "
                "! same_checkpoint_time \"$root_latest\" \"$processor_latest\"; then\n"
            "            echo \"Reconstruction did not produce a complete root "
                "checkpoint matching t=$processor_latest. Old partitions were "
                "preserved; refusing redecomposition.\" >&2\n"
            "            exit 10\n"
            "        fi\n"
            "    fi\n"
            ...
            "        echo \"Redecomposition did not produce a complete checkpoint "
                "on all $processes requested partitions. No solver stage was "
                "started.\" >&2\n"
            "        exit 10\n"
            "    fi\n"
            "    ensure_processor_time_metadata \"$repartitioned_time\" || exit 10\n"
            "    echo \"Repartitioned complete checkpoint t=$repartitioned_time "
                "for $processes requested processes.\"\n"
            "    summary \"repartitioned_checkpoint time=$repartitioned_time "
        ...
        << options.airflow_checkpoint_interval << "\n"
            ...
            "{\n"
            "    local keep=\"$checkpoint_retention\" processor_root candidate target rank remove_count index\n"
            ...
            "        is_restartable_processor_time \"$candidate\" && "
        ...
        if(options.use_multirate_thermal) {
            ...
            output <<
                ...
                "    {\n"
                "        awk '/^[[:space:]]*internalField[[:space:]]/{ getline; "
                    "print $1; exit }' \"$1\" 2>/dev/null\n"
                "    }\n"
                ...
                "    {\n"
                "        local latest reference_time rank source target current_field "
                    "source_count current_count field spatial_output spatial_status=0 "
                    "velocity_rms_delta velocity_rms_reference\n"
                "        accepted_airflow_relative_rms=\"\"\n"
                ...
                "            if [[ ! \"$source_count\" =~ ^[0-9]+$ || "
                    "! \"$current_count\" =~ ^[0-9]+$ || "
                    "\"$source_count\" != \"$current_count\" ]]; then\n"
                "                echo \"Accepted airflow reference is incomplete or incompatible "
                    "with the current decomposition at rank $rank "
                    "(referenceCells=${source_count:-unknown}, "
                    "currentCells=${current_count:-unknown}); rebuilding it.\" >&2\n"
                "                for ((rank=0; rank<processes; ++rank)); do\n"
                "                    rm -f -- \"$case_dir/processor${rank}/${latest}/fluid/UPrevious\"\n"
                "                done\n"
                "                record_accepted_airflow_reference || return 1\n"
            ...
            if(options.stop_when_thermally_converged) {
                ...
                "    {\n"
                "        local maximum_root average_root average_file fluid_average_root "
                    "region line sample_time checkpoint_time peak previous_time "
                    "previous_peak fluid_average previous_fluid_average elapsed delta scaled_delta value previous "
                    "index=0 maximum_peak_delta=0 maximum_average_delta=0 "
                    "scaled_average_delta scaled_fluid_max_delta expected_state_values "
                ...
                "        done\n"
                "        if [[ ! -f \"$thermal_convergence_state\" ]]; then\n"
                "            printf '%s %s %s' \"$checkpoint_time\" \"$peak\" \"$fluid_average\" > "
                    "\"$thermal_convergence_state\"\n"
                "            printf ' %s' \"${maxima[@]}\" >> "
                    "\"$thermal_convergence_state\"\n"
                "            printf ' %s' \"${averages[@]}\" >> "
                ...
                "            return 1\n"
                "        fi\n"
                "        scaled_fluid_max_delta=$(awk -v a=\"$peak\" "
                    "-v b=\"$previous_peak\" -v reference=\""
                    << options.thermal_convergence_reference_interval
                    << "\" -v elapsed=\"$elapsed\" 'BEGIN { d=a-b; "
                    "if(d<0)d=-d; print d*reference/elapsed }')\n"
                ...
                "        echo \"Thermal convergence metrics: "
                    ...
                    << options.thermal_convergence_reference_interval
                    << "s, fluidMaximumChange=$scaled_fluid_max_delta K/"
                    << options.thermal_convergence_reference_interval
                    << "s (diagnostic), controllingPeakRegion=$controlling_peak_region, "
            ...
            output <<
                ...
                "            stage_plan=$(awk -v maximum=\"$stage_max_dt\" "
                    "-v remaining=\"$interval\" -v checkpoint=\""
                << options.airflow_checkpoint_interval
                << "\" 'BEGIN { n=int(remaining/maximum); "
                    "if(n*maximum<remaining-1e-12)n++; if(n<1)n=1; "
                    "blocks=int(remaining/checkpoint); "
                    "if(blocks*checkpoint<remaining-1e-12)blocks++; "
                    "if(blocks<1)blocks=1; per=int(n/blocks); "
                    "if(per*blocks<n)per++; if(per<1)per=1; n=per*blocks; "
            ...
            "            if ! run_latest_courant_postprocess postflight_output; then\n"
                ...
                "            for ((rank=0; rank<processes; ++rank)); do\n"
                "                source_field=\"$stage_velocity_reference/"
                    "processor${rank}/U\"\n"
                ...
                "            done\n"
                "        fi\n"
                "        if [[ \"$thermal_only\" == \"true\" && \"$unqualified_thermal\" != true ]]; then\n"
            ...
            if(options.use_adaptive_airflow_refresh) {
                ...
                output <<
                    ...
                    "awk -v t=\"$air_exchange_last_time\" -v f=\"$air_exchange_last_flow\" "
                        "-v x=\"$air_exchange_fraction\" -v start=\"$initial_start\" "
                        "-v now=\"$current\" 'BEGIN { "
                    ...
                    "            echo \"Restored cumulative initial air exchange: "
                        "fraction=$air_exchange_fraction at t=$air_exchange_last_time s.\"\n"
                    "        else\n"
                    ...
                    << "\" -v limit=\"$initial_limit\" 'BEGIN { x=now+interval; "
                        "if(x>limit)x=limit; printf \"%.17g\", x }')\n"
                    ...
                    "                exchange_was_incomplete=false\n"
                    "                if [[ ! -f \"$mapped_state_marker\" ]] && ! awk -v completed=\"$air_exchange_fraction\" -v required=\""
                    ...
                    "                    air_exchange_fraction=$(awk -v accumulated=\"$air_exchange_fraction\" "
                        "-v previous_time=\"$air_exchange_last_time\" -v now=\"$current\" "
                        "-v previous_flow=\"$air_exchange_last_flow\" "
                        "-v flow=\"$latest_one_way_boundary_mass_flow\" -v volume=\""
                    << fluid_volume_m3 << "\" -v rho=\""
                    << mesh.get_env().get_rho()
                    ...
                    "                    air_exchange_state_tmp=\"${initial_exchange_state}.tmp.$$\"\n"
                    "                    printf '%s %s %s\\n' \"$air_exchange_last_time\" "
                        "\"$air_exchange_last_flow\" \"$air_exchange_fraction\" > "
                        "\"$air_exchange_state_tmp\"\n"
                    "                    mv -f \"$air_exchange_state_tmp\" \"$initial_exchange_state\"\n"
                    "                    echo \"Cumulative initial air exchange: "
                        "fraction=$air_exchange_fraction at t=$current s.\"\n"
                    "                    summary \"initial_air_exchange_progress current=$current "
                        "fraction=$air_exchange_fraction oneWayMassFlow=$air_exchange_last_flow\"\n"
                    "                fi\n"
                    ...
                    "                    summary \"initial_air_exchange_advance current=$current "
                        "target=$exchange_target completedFraction=$air_exchange_fraction\"\n"
                    "                    continue\n"
                    "                fi\n"
            ...
            if(options.use_adaptive_airflow_refresh) {
                output <<
                    "    if [[ \"$unqualified_thermal\" == true ]]; then\n"
                    "        restore_full_fan_options\n"
                    "        restore_full_solid_options\n"
                    "        summary \"UNQUALIFIED thermal test start=$current end=$requested_end airflow_accepted=false\"\n"
                    "        stage true \"$requested_end\" "
                    << options.frozen_flow_maximum_courant_number << ' '
                    << options.frozen_flow_maximum_time_step
                    << " \"UNQUALIFIED exploratory thermal-only stage\" 1\n"
                    "        summary \"UNQUALIFIED thermal test complete time=$current airflow_accepted=false\"\n"
                    "        echo \"UNQUALIFIED thermal test reached $current; airflow remains unqualified.\"\n"
                    "        exit 0\n"
                    "    fi\n"
                    "    if [[ -f \"$initial_convergence_marker\" ]]; then\n"
