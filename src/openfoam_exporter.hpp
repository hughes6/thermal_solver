...
functions
{
    ...
    static void write_parallel_run_script(
        ...
        << options.airflow_checkpoint_interval << "\n"
            ...
            "    # OpenFOAM can stop one fixed timestep below a decimal "
                "endpoint after accumulated binary round-off.\n"
            "    if ! awk -v actual=\"$actual\" -v target=\"$target\" "
                "-v step=\"$warm_start_maximum_time_step\" "
                ...
                "if(step>tolerance)tolerance=step*1.001; "
            ...
            if(options.use_adaptive_airflow_refresh) {
                ...
                output <<
                    ...
                    "        if ! awk -v a=\"$current\" "
                        ...
                        "tol=1e-9*s; if(tol>1e-8)tol=1e-8; "
                        "exit !(a>=b-tol) }'; then\n"
                    "            if [[ \"${THERMAL_ALLOW_UNVERIFIED_AIRFLOW:-0}\" == \"1\" ]]; then\n"
                    "                echo \"WARNING: airflow did not pass the acceptance audit; continuing only as an UNVERIFIED airflow thermal screening.\" >&2\n"
                    "                if ! record_accepted_airflow_reference; then\n"
                    "                    echo \"Unable to preserve the unverified airflow reference.\" >&2\n"
                    "                    return 3\n"
                    "                fi\n"
                    "                mkdir -p provenance\n"
                    "                printf 'UNVERIFIED_AIRFLOW_SCREENING time=%s reason=initial_airflow_audit_not_converged\\n' \"$current\" > provenance/UNVERIFIED_AIRFLOW_SCREENING\n"
                    "                rm -f \"$initial_pending_marker\" \"$mapped_state_marker\" \"$initial_exchange_state\" \"$initial_physical_settling_marker\"\n"
                    "                fan_options_source=\"$full_fan_options\"\n"
                    "                restore_full_fan_options\n"
                    "                echo \"Restored full fluid heat sources for provisional thermal screening. Results are hotspot/trend evidence only, not validated temperatures.\"\n"
