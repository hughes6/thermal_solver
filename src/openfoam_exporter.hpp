...
private:
    ...
    static void write_parallel_run_script(
        ...
        output <<
            ...
            "    # warm_interval is measured in simulation seconds, so force\n"
            "    # adjustableRunTime even when a previous multirate airflow\n"
            "    # stage left writeControl set to timeStep.\n"
            "    \"$foam_launcher\" foamDictionary "
                "\"$case_dir/system/controlDict\" "
            ...
            "        # preflight. Use a conservative maxCo-scaled fixed cap\n"
            "        # and divide every ramp interval exactly.\n"
            "        ramp_cap=$(awk -v flow_max=\""
            << options.airflow_maximum_time_step
            << "\" -v co=\""
            << options.maximum_courant_number
            << "\" 'BEGIN { scale=(co<10?co/10:1); print flow_max*scale }')\n"
