        ...
        if(model.openfoam_solver.enabled) {
            ...
            if(cfg.use_multirate_thermal && cfg.use_adaptive_airflow_refresh) {
                std::cout
                    ...
                    << "UNQUALIFIED EXPLORATORY THERMAL TEST (only for a quick trend check):\n"
                    << "  # A stopped --cold-flow-seed case may be imported before qualification. This is not a validated result.\n"
                    << "  # Set SEED_CASE and CHECKPOINT to the donor and exact processor checkpoint; use a fresh target export.\n"
                    << "  OPENFOAM_LAUNCHER=env bash ./create_thermal_branch_from_cold_flow_seed.sh \"$SEED_CASE\" \"$PWD\" "
                    << cfg.parallel_processes << " --allow-unqualified \"$CHECKPOINT\"\n"
                    << "  THERMAL_SOLVER_OPENFOAM_ENV_READY=1 OPENFOAM_LAUNCHER=env bash ./run_parallel.sh "
                    << cfg.parallel_processes << " --unqualified-thermal 0.5 2>&1 | tee -a unqualified-thermal.stdout.log\n"
                    << "  # This holds imported airflow fixed, copies no temperatures, skips acceptance/refresh, and keeps no convergence marker.\n"
                    << "  # Full instructions: UNQUALIFIED_THERMAL_TEST.md.\n\n"
