...
def main() -> None:
    ...
    for path, block in iter_named_leaves(data):
        ...
        if "T" not in block.cell_data and "T" in block.point_data:
            block = block.point_data_to_cell_data(pass_point_data=True)
        if "T" not in block.cell_data or "phi" not in block.cell_data:
            raise SystemExit(
                f"Patch {'/'.join(path)} does not contain cell arrays T and phi.\n"
