#ifndef THERMAL_LOGGER_HPP
#define THERMAL_LOGGER_HPP
...
struct SummaryRequest {
    ...
    SummaryRequest(std::string n, LogVariable var, CellSelection sel, bool min, bool max, bool avg, bool rms, bool stddev) :
        name(n), variable(var), selection(sel), log_min(min), log_max(max), log_average(avg), log_rms(rms), log_standard_deviation(stddev) {}
        
    std::string name;

    LogVariable variable = LogVariable::Temperature;
...
public:
    explicit SimulationLogger(LoggingConfig config)  : config_(std::move(config)) {}
    ...
    void initialize(const Mesh& mesh) {
        if(initialized_) {
            throw std::runtime_error( "SimulationLogger is already initialized.");
        }
    ...
    void log(const Mesh& mesh, int timestep, double time) {
        ...
        if(config_.enable_summary_logging && config_.summary_interval > 0 && timestep % config_.summary_interval == 0) {
            log_summary(mesh, timestep, time);
        }

        if(config_.enable_probe_logging &&  config_.probe_interval > 0 &&  timestep % config_.probe_interval == 0) {
            log_probes(mesh, timestep, time);
        }
    }

    void close() {
        if(field_file_.is_open()) { field_file_.close(); }
...
private:
    ...
    void validate_config() const {
        if(config_.enable_field_logging && config_.field_interval <= 0) {
            throw std::invalid_argument("Field logging interval must be greater than zero.");
        }
        if(config_.enable_summary_logging && config_.summary_interval <= 0) {
            throw std::invalid_argument( "Summary logging interval must be greater than zero.");
        }
    ...
    static std::string variable_name(LogVariable variable) {
        switch(variable) {
            case LogVariable::Temperature: return "T";
            case LogVariable::Pressure: return "pressure";
            case LogVariable::VelocityX: return "vx";
            case LogVariable::VelocityY: return "vy";
            case LogVariable::VelocityZ: return "vz";
            case LogVariable::VelocityMagnitude: return "velocity_magnitude";
            case LogVariable::Density: return "rho";
            case LogVariable::SpecificHeat: return "cp";
            case LogVariable::Conductivity: return "k";
            case LogVariable::HeatGeneration: return "qdot";
            case LogVariable::ReynoldsNumber: return "Re";
            case LogVariable::ConvectionCoefficient: return "h";
        }
        throw std::runtime_error("Unknown LogVariable.");
    }
    ...
    static double get_log_value(const Cell& cell, LogVariable variable) {
        switch(variable) {
            case LogVariable::Temperature: return cell.get_T();
            case LogVariable::Pressure: return cell.get_pressure();
            case LogVariable::VelocityX: return cell.get_vx();
            case LogVariable::VelocityY: return cell.get_vy();
            case LogVariable::VelocityZ: return cell.get_vz();
            case LogVariable::VelocityMagnitude: { return cell.get_vmag();}
            case LogVariable::Density: return cell.get_rho();
            case LogVariable::SpecificHeat: return cell.get_cp();
            case LogVariable::Conductivity: return cell.get_k();
            case LogVariable::HeatGeneration: return cell.get_qdot();
            case LogVariable::ReynoldsNumber: return Convection::local_reynolds(cell.get_vmag(), std::min({cell.get_dx(), cell.get_dy(), cell.get_dz()}), cell.get_rho(), cell.get_mu());
            case LogVariable::ConvectionCoefficient: return cell.get_h();
        }
    ...
    static bool cell_matches_selection(const Cell& cell, CellSelection selection) {
        switch(selection) {
            case CellSelection::All: return true;
            case CellSelection::AirOnly: return cell.is_air();
            case CellSelection::SolidOnly: return cell.is_solid();
            case CellSelection::FluidOnly: return cell.is_fluid();
            case CellSelection::HeatGeneratingOnly: return cell.get_qdot() > 0.0;
        }
        return false;
    }
    ...
    static SummaryStatistics compute_summary(const Mesh& mesh, const SummaryRequest& request) {
        SummaryStatistics statistics;

        for(const Cell& cell : mesh.get_cells()) {
            if(!cell_matches_selection(cell, request.selection)) {
                continue;
            }
            statistics.add(get_log_value(cell, request.variable));
        }
        return statistics;
    }
