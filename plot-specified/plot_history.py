...
def main():
    ...
    if a.probe:
        ...
        if not paths:
            fallback=Path("simulation_output/summary.csv")
            if fallback.exists():
                a.summary=str(fallback)
                print("no probe files found: plotting summary")
            else:
                raise FileNotFoundError(
                    "No probe files matched and simulation_output/summary.csv does not exist.")
    ...
    ax.set_xlabel('Time [s]'); ax.set_ylabel('Value'); ax.grid(True)
