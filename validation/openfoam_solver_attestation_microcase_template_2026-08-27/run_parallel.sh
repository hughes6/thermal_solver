...
if [[ -z "$semi_frozen_solver" || ! -f "$semi_frozen_solver" || ! -x "$semi_frozen_solver" ]]; then
    echo "ERROR: required custom OpenFOAM solver 'semiFrozenChtMultiRegionFoam' was not found as an executable file in PATH after OpenFOAM environment setup. No case lock or case write was attempted." >&2
    exit 14
fi
if ! command -v grep >/dev/null 2>&1; then
    echo "ERROR: required binary marker checker 'grep' is unavailable after OpenFOAM environment setup. No case lock or case write was attempted." >&2
    exit 14
fi
if ! grep -aFq -- "$solver_mode_policy_marker" "$semi_frozen_solver"; then
    echo "ERROR: custom OpenFOAM solver '$semi_frozen_solver' does not contain required mode-policy marker '$solver_mode_policy_marker'. The binary is stale or unverified; no case lock or case write was attempted." >&2
    exit 14
fi
echo "Verified custom OpenFOAM solver policy: path=$semi_frozen_solver marker=$solver_mode_policy_marker"
